...
def process_text_chunk(text: str, language: str = "a", skip_phonemize: bool = False) -> List[int]:
    ...
    logger.debug(f"Total processing took {total_time * 1000:.2f}ms for chunk: '{text[:50]}{'...' if len(text) > 50 else ''}'")
...
async def yield_chunk(text: str, tokens: List[int], chunk_count: int) -> Tuple[str, List[int]]:
    ...
    logger.debug(f"Yielding chunk {chunk_count}: '{text[:50]}{'...' if len(text) > 50 else ''}' ({len(tokens)} tokens)")
    return text, tokens
...
async def smart_split(
    ...
    for sentence, tokens, count in sentences:
        ...
        if count > max_tokens:
            ...
            if current_chunk:
                ...
                chunk_count += 1
                logger.debug(f"Yielding chunk {chunk_count}: '{chunk_text[:50]}{'...' if len(text) > 50 else ''}' ({current_count} tokens)")
            ...
            for j in range(0, len(clauses), 2):
                ...
                if clause_count + count <= max_tokens and clause_count + count <= settings.target_max_tokens:
                    ...
                    if clause_chunk:
                        ...
                        chunk_count += 1
                        logger.debug(f"Yielding clause chunk {chunk_count}: '{chunk_text[:50]}{'...' if len(text) > 50 else ''}' ({clause_count} tokens)")
            ...
            if clause_chunk:
                chunk_text = " ".join(clause_chunk)
                chunk_count += 1
                logger.debug(f"Yielding final clause chunk {chunk_count}: '{chunk_text[:50]}{'...' if len(text) > 50 else ''}' ({clause_count} tokens)")
                yield chunk_text, clause_tokens
        ...
        # Regular sentence handling
            ...
            chunk_text = " ".join(current_chunk)
            chunk_count += 1
            logger.info(f"Yielding chunk {chunk_count}: '{chunk_text[:50]}{'...' if len(text) > 50 else ''}' ({current_count} tokens)")
            yield chunk_text, current_tokens
    ...
    if current_chunk:
        chunk_text = " ".join(current_chunk)
        chunk_count += 1
        logger.info(f"Yielding final chunk {chunk_count}: '{chunk_text[:50]}{'...' if len(text) > 50 else ''}' ({current_count} tokens)")
        yield chunk_text, current_tokens
